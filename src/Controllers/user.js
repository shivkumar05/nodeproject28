...
const createRoutine = async function (req, res) {
    try {
        ...
        let { drills, date, time, category, drill_id, repetation, sets, userId, routineId, end_date } = data;
        data.userId = userid;
...
const getRoutine = async function (req, res) {
    try {
        ...
        const getDrills = await routineModel.find({userId: userid}).sort({ time: data.time })
...
let getAllUsers = async function (req, res) {
    try {
        ...
        for (var i = 0; i < allUser.length; i++) {
            ...
            var UserProfile = await profileModel.find({ userId: asd }).select({ _id: 0, createdAt: 0, updatedAt: 0, __v: 0 });
...
//       return { ...user.toJSON(), ...profile.toJSON() };
//     });
//     res.json(data);
//   } catch (error) {
//     console.error(error);
